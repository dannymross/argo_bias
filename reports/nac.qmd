...
#| output: asis
#| code-summary: "RG climatology by year and month, native 1° grid vs interpolated to 1/12°, and the GLORYS truth - RG climatology anomaly"
...
def show_clim_map(kind, d):
    ...
    if kind == "native":
        return ohc_bias.plot_monthly_cell_maps(
            _clim_A_native_cells, d, vmin=cmin, vmax=cmax, width=TABSET_FIGW, ncols=6,
            title=f"RG climatology {d} by year/month (native 1°)")
    if kind == "interp":
        return ohc_bias.plot_monthly_cell_maps(
            _clim_A_interp_cells, d, vmin=cmin, vmax=cmax, width=TABSET_FIGW, ncols=6,
            title=f"RG climatology {d} by year/month (bilinear -> 1/12°)")
    if kind == "anom":
        return ohc_bias.plot_monthly_cell_maps(
            truth_anom_cells_all, d, vmin=-ANOM_VLIM[d], vmax=ANOM_VLIM[d], cmap="RdBu_r",
            cbar_label=f"{d} anomaly (GJ m$^{{-2}}$)", width=TABSET_FIGW, ncols=6,
