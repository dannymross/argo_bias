...
def deploy_float_grid(
    ...
    Mode 1 (top_left+bottom_right+nfloats): aspect-ratio-preserving grid.
    Mode 2 (top_left+bottom_right+spacing_deg): fixed degree spacing.
    Mode 3 (top_left+spacing_deg[+nfloats]): expands to the North Atlantic
    default bounds, then caps to the first nfloats ocean points (row-major,
    W->E, N->S) if given.

    Deterministic for fixed inputs, so parallel workers can each rebuild the
    same plan and take a slice via :func:`select_floats`. ``cell_deg``
    deploys one float per analysis-cell centre using the same floor-binning
    as :func:`ohc.grid_cells`, tiling the deployment onto the analysis grid
    exactly (overrides nfloats/spacing_deg; requires ``bottom_right``).
    ``snap_deg`` snaps positions to the nearest model-grid multiple (e.g.
    1/12 for GLORYS12); idempotent for positions already on the grid.
    """
...
def fetch_velocity_data(
    ...
    compute nodes without network access or the package installed.
    """
...
def fetch_velocity_months(
    ...
    interrupted downloads resume cleanly. Files are named
    ``<file_prefix>_YYYY_MM.nc``; returns ``(paths, glob)`` -- pass the glob
    to :func:`build_velocity_field`.
    """
    import calendar
...
def build_velocity_field(
    ...
    Always pass a file path/glob (e.g. ``"data/velocity/*.nc"``), not an open
    ``Dataset`` -- a path routes through ``FieldSet.from_netcdf(deferred_load=True)``
    so parcels reads time chunks lazily; an open ``Dataset`` forces an eager
    full-field load and won't scale to multi-year fields.
    """
...
def run_simulation(
    ...
    ``record_seconds`` must be a multiple of ``step_seconds``. Returns the
    ``.zarr`` output path; ``out_file`` defaults to a random name if omitted.
    """
    out_dir = os.path.expanduser(out_dir)
    os.makedirs(out_dir, exist_ok=True)
